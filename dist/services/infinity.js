import { b as _defineProperty } from '../_rollupPluginBabelHelpers-L-9AK1w0.js';
import Service from '@ember/service';
import { c as checkInstanceOf, a as convertToArray, I as InfinityModel, o as objectAssign, p as paramsCheck } from '../infinity-model-CG-0ABLy.js';
import InfinityPromiseArray from '../lib/infinity-promise-array.js';
...
const ALLOWED_KEYS = ['perPage', 'perPageParam', 'startingPage', 'firstPage', 'totalPagesParam', 'countParam', 'infinityCache', 'filter', 'storeFindMethod', 'meta'];
...
class Infinity extends Service {
  /**
  ...
  */
  ...
  constructor(...args) {
    super(...args);
    _defineProperty(this, "infinityModels", void 0);
    /**
      @private
      ...
      @default 0
    */
    _defineProperty(this, "_previousScrollHeight", 0);
  ...
  */
  ...
  loadNextPage(infinityModel, increment = 1) {
    ...
    return this._requestNextPage(infinityModel, increment).then(newObjects => this._afterInfinityModel(newObjects, infinityModel)).then(newObjects => this._doUpdate(newObjects, infinityModel)).then(infinityModel => {
      if (increment === 1) {
        // scroll down to load next page
        infinityModel.incrementProperty('currentPage');
      } else {
        if (typeof FastBoot === 'undefined') {
          let viewportElem = infinityModel._scrollable ? document.querySelector(infinityModel._scrollable) : document.scrollingElement || document.documentElement;
  ...
  _calculateHeight(infinityModel) {
    if (typeof FastBoot === 'undefined') {
      ...
      const viewportElem = isScrollable ? document.querySelector(infinityModel._scrollable) : document.documentElement;
